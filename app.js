...
function renderIndexGames() {
    const container = document.getElementById('indexGames');
    if (!container) return;
    const games = getGames().slice(0, 3);
    if (games.length === 0) {
        container.innerHTML = emptyState('🎮', 'Belum ada game. Tambahkan melalui halaman Games!');
        return;
    }
    container.innerHTML = games.map(g => renderGameCard(g, false)).join('');
}
